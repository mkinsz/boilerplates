...
### Webpack中易混淆概念
> Tip: [参考](https://www.cnblogs.com/skychx/tag/Webpack/)

#### module, chunk, bundle的区别
webpack官网对bundle和chunk的解释太过抽象，所以我们可以通过例子来详细了解一下:
...
一般来说一个 chunk 对应一个bundle，比如上图中的`utils.js -> chunks 1 -> utils.bundle.js`; 但也有例外, 比如说上图中，我就用 `MiniCssExtractPlugin` 从chunks 0中抽离出了`index.bundle.css`文件。
...
哈希一般是结合CDN缓存来使用的, 如果文件内容改变的话, 那么对应文件哈希值也会改变, 对应的html引用的url地址也会随之改变，触发CDN服务器从源服务器上拉取对应数据, 进而更新本地缓存.

1. `hash`: 计算是跟整个项目的构建相关，我们做一个简单的 demo
文件目录如下：
```js
src/
├── index.css
...
2. `chunkhash`: 因为 hash 是项目构建的哈希值，项目中如果有些变动，hash 一定会变，比如说我改动了 utils.js 的代码，index.js 里的代码虽然没有改变，但是大家都是用的同一份 hash。hash 一变，缓存一定失效了，这样子是没办法实现 CDN 和浏览器缓存的。
...
```
构建结果如下：
![avatar](./img/chunkhash1.jpg)
我们可以看出，chunk 0 的 hash 都是一样的，chunk 1 的 hash 和上面的不一样。
假设我又把 utils.js 里的 cube() 函数去掉，再打包：
![avatar](./img/chunkhash2.jpg)
对比可以发现，只有 chunk 1 的 hash 发生变化，chunk 0 的 hash 还是原来的。

3. `contenthash`: 我们更近一步，index.js 和 index.css 同为一个 chunk，如果 index.js 内容发生变化，但是 index.css 没有变化，打包后他们的 hash 都发生变化，这对 css 文件来说是一种浪费。如何解决这个问题呢？
...
![avatar](./img/contenthash1.jpg)
![avatar](./img/contenthash2.jpg)
![avatar](./img/contenthash3.jpg)
...
#### sourse-map中eval, cheap, inline 和 module的区别
...
![avatar](./img/inline-source-map.jpg)
...
1. `filename`: 这是一个很常见的配置，就是对应于 entry 里面的输入文件，经过webpack 打包后输出文件的文件名。比如说经过下面的配置，生成出来的文件名为 index.min.js。
...
2. `chunkFilename`: 指未被列在entry中，却又需要被打包出来的chunk文件的名称。一般来说，这个 chunk 文件指的就是要懒加载的代码。
...
#### webpackPrefetch, webpackPreload 和 webpackChunkName的区别
1. `webpackChunkName`: 前面举了个异步加载 lodash 的例子，我们最后把 output.chunkFilename 写死成 bundle.js。在我们的业务代码中，不可能只异步加载一个文件，所以写死肯定是不行的，但是写成 [name].bundle.js 时，打包的文件又是意义不明、辨识度不高的 chunk id。
```js
{
    entry: {
        index: "../src/index.js"
...
```
![avatar](./img/webpackChunkName1.jpg)
...
```
这时候打包生成的文件是这样的：
![avatar](./img/webpackChunkName2.jpg)
